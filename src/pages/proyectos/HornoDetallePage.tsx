...
import { hornos } from "./data/hornos";

export const HornoDetallePage = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const obra = hornos.find((h) => h.id === id);

  if (!obra) {
    return (
      <div className="min-h-screen flex items-center justify-center text-sky-950 text-2xl">
        Obra no encontrada.
      </div>
    );
  }
      ...
      <main>
        ...
        <div className="relative h-[420px] md:h-[580px] w-full overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col justify-end h-full max-w-[1600px] mx-auto px-[34px] md:px-20 pb-16">
            <button
              onClick={() => navigate("/proyectos/hornos")}
              className="text-white/70 text-sm mb-6 hover:text-white text-left"
            >
              ← Volver a Hornos
            </button>
            <span className="block text-white/70 text-sm mb-3">{obra.cliente} — {obra.ubicacion}</span>
            <h1 className="text-white text-3xl md:text-5xl font-light leading-tight tracking-[-1px] max-w-[900px]">
              {obra.titulo}
            </h1>
          </div>
        </div>
        ...
        <div className="bg-white py-[60px] md:py-[80px]">
          <div className="mx-auto max-w-[1600px] px-[34px] md:px-20 flex flex-col md:flex-row gap-10 md:gap-24">
            ...
            <div className="md:w-3/4">
              <div className="text-zinc-600 text-base md:text-lg leading-relaxed whitespace-pre-line">
                {obra.descripcion}
              </div>
            </div>
          </div>
