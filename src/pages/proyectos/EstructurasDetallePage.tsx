...
export const EstructurasDetallePage = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const obra = estructuras.find((h) => h.id === id);
      ...
      <main>
        <div className="relative h-[420px] md:h-[580px] w-full overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col justify-end h-full max-w-[1600px] mx-auto px-[34px] md:px-20 pb-28 md:pb-36">
            <button onClick={() => navigate("/proyectos/estructuras")} className="text-white/70 text-sm mb-6 hover:text-white text-left">
              ← Volver a Estructuras y Equipos
            </button>
